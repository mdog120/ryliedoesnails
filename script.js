...
let liveOpenings = {
  "2026-07-27": ["9:00 AM", "1:00 PM", "5:15 PM"],
  "2026-07-29": ["9:00 AM"],
  "2026-07-30": ["10:00 AM"],
  "2026-08-03": ["9:00 AM", "1:00 PM", "5:15 PM"],
  "2026-08-05": ["9:00 AM"],
  "2026-08-06": ["10:00 AM"],
  "2026-08-10": ["9:00 AM", "1:00 PM", "5:15 PM"],
  "2026-08-12": ["9:00 AM"],
  "2026-08-13": ["10:00 AM"],
};
...
if (calendar) {
  ...
  function renderCalendar() {
    ...
    calendar.querySelectorAll("[data-date]").forEach((day) =>
      day.addEventListener("click", () => {
        bookingSelection = { date: day.dataset.date, time: "" };
        chosenSlot.value = "";
        ...
        timeSlots.querySelectorAll(".time-slot").forEach((button) =>
          button.addEventListener("click", () => {
            bookingSelection.time = button.dataset.time;
            timeSlots
              .querySelectorAll(".time-slot")
              .forEach((slot) =>
                slot.classList.toggle("is-selected", slot === button),
              );
            chosenSlot.value = `${dateLabel(bookingSelection.date)} at ${bookingSelection.time}`;
    ...
    title.textContent = new Date(
      `${calendarWeeks[week]}T12:00:00`,
    ).toLocaleDateString("en-US", { month: "long", year: "numeric" });
  }
  window.rylieAvailability = (data) => {
    if (data?.openings && Object.keys(data.openings).length) {
      liveOpenings = data.openings;
      renderCalendar();
    }
  };
  ...
  if (endpoint) {
    ...
    script.src = `${endpoint}?callback=rylieAvailability`;
...
bookingForm?.addEventListener("submit", (event) => {
  ...
  if (!endpoint) {
    status.textContent =
      "The website is ready, but it needs the free Google Apps Script link before requests can send to Rylie.";
    return;
  }
  const send = (design) => {
    const payload = {
      name: bookingForm.elements.name.value,
      age: bookingForm.elements.age.value,
      contact: bookingForm.elements.contact.value,
      payment: bookingForm.elements.payment.value,
      service: bookingForm.elements.service.value,
      shape: bookingForm.elements.shape.value,
      notes: bookingForm.elements.notes.value,
      appointmentDate: bookingSelection.date,
      appointmentTime: bookingSelection.time,
      design,
    };
    fetch(endpoint, {
      method: "POST",
  ...
  if (file.size > 5 * 1024 * 1024) {
    status.textContent =
      "Please choose an inspiration image smaller than 5 MB.";
    return;
  }
